...
def decode_detections(dets, info, calibs, cls_mean_size, threshold):
    ...
    for i in range(dets.shape[0]):  # batch
        ...
        for j in range(dets.shape[1]):  # max_dets
            ...
            padding = (info['img_size'][i][1] - (info['img_size'][i][1] / info['height_crop'][i])) // 2
            # 2d bboxs decoding
            x = dets[i, j, 2] * info['img_size'][i][0]
            y = dets[i, j, 3] * (info['img_size'][i][1] / info['height_crop'][i]) + padding
            w = dets[i, j, 4] * info['img_size'][i][0]
            h = dets[i, j, 5] * (info['img_size'][i][1] / info['height_crop'][i])
            bbox = [x-w/2, y-h/2, x+w/2, y+h/2]

            # 3d bboxs decoding
            # depth decoding
            depth = dets[i, j, 6] / info['canonical_scale'][i]
            ...
            y3d = dets[i, j, 35] * (info['img_size'][i][1] / info['height_crop'][i]) + padding
