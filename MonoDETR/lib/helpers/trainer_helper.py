...
class Trainer(object):
    ...
    def train_one_epoch(self, epoch):
        ...
        for batch_idx, (inputs, calibs, targets, info) in enumerate(self.train_loader):
            ...
            #print(detr_losses_dict)
            #print(len(detr_losses_dict))
            ...
            #print(detr_losses_dict_weighted)
            #print(len(detr_losses_dict_weighted))
    ...
    def prepare_targets(self, targets, batch_size):
        ...
        key_list = ['labels', 'scores', 'boxes', 'calibs', 'depth', 'size_3d', 'heading_bin', 'heading_res', 'boxes_3d']
        for bz in range(batch_size):
            ...
            # Some KITTI label files don't include a score column; in that case Object3d.score is -1.
            # Treat missing scores as 1.0 so score-weighted training matches normal supervised training.
            if 'scores' in target_dict:
                scores = target_dict['scores'].to(torch.float32)
                scores = torch.where(scores < 0, torch.ones_like(scores), scores)
                scores = torch.clamp(scores, 0.0, 1.0)
                target_dict['scores'] = scores
