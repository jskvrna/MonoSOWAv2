...
const int32_t N_TESTIMAGES = 7480; //7480
...
bool eval(string result_sha,Mail* mail){
  ...
  string gt_dir         = "/path/to/datasets/kitti/training/label_2";
  string result_dir     = "/path/to/labels/labels_occlusion_mask/";
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    ...
    printf("Checking 2D evaluation (%s) ...", CLASS_NAMES[c].c_str());
    if (eval_image[c]) {
      printf("Starting 2D evaluation (%s) ...", CLASS_NAMES[c].c_str());
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    CLASSES cls = (CLASSES)c;
    printf("Checking bird's eye evaluation (%s) ...", CLASS_NAMES[c].c_str());
    if (eval_ground[c]) {
      printf("Starting bird's eye evaluation (%s) ...", CLASS_NAMES[c].c_str());
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    CLASSES cls = (CLASSES)c;
    //printf("Checking 3D evaluation (%s) ...", CLASS_NAMES[c].c_str());
    if (eval_3d[c]) {
      printf("Starting 3D evaluation (%s) ...", CLASS_NAMES[c].c_str());
...
int32_t main (int32_t argc,char *argv[]) {
  ...
  if (eval(result_sha,mail)) {
    printf("Your evaluation results are available at:");
    printf("http://www.cvlibs.net/datasets/kitti/user_submit_check_login.php?benchmark=object&user=%s&result=%s",argv[2], result_sha.c_str());
  } else {
