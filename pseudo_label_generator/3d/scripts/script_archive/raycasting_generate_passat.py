...
for angle_rad in np.linspace(0., 2*np.pi - (2*np.pi)/nclouds, nclouds): #Per 10 degrees
    ...
    if wo_windows:
        mesh = o3d.io.read_triangle_mesh("../../data/passat_nowindows2.gltf") #Read mesh of fiat uno converted via blender
    else:
        mesh = o3d.io.read_triangle_mesh(
            "../../data/passat2.gltf")  # Read mesh of fiat uno converted via blender
    ...
    if wo_windows:
        ...
        # Scale vertices in each dimension separately, works, eventhough it looks wrong
        vertices[:, 1] *= scale_height
        vertices[:, 2] *= scale_lenght
        vertices[:, 0] *= scale_width
    ...
    T = np.eye(4)
    if wo_windows:
        T[:3, :3] = o3d.geometry.get_rotation_matrix_from_zxy(
            (0, 1.5 * np.pi, 0))  # X rotation to make the car as in the dataset and pointing forward as the reference car
    else:
        T[:3, :3] = o3d.geometry.get_rotation_matrix_from_zxy(
            (np.pi, 0, 0))
    mesh.transform(T)
    ...
    T = np.eye(4)
    T[:3, 3] = (0, 0.25, 5) #Move it a little bit in front of the camera. The Y or the height was chosen empirically to not get many points from the roof, since in real life, we usually lack those points
    mesh.transform(T)
    ...
    point_cloud = point_cloud.translate([0, -0.25, -5])
    #saves the pointcloud
    if wo_windows:
        file_name = "../data/pcloud_filtered_passat_nw/" + str(int(degrees)) + ".pcd"
    else:
        file_name = "../data/pcloud_filtered_passat/" + str(int(degrees)) + ".pcd"
...
#Because the mesh is not completely "waterproof" especially around wheels, this method removes points, that are seen but would make mess in the optimization
#point_cloud = pcd.remove_statistical_outlier(10, 1.)[0]
point_cloud = pcd
#saves the pointcloud
if wo_windows:
    file_name = "../data/pcloud_filtered_passat_nw/" + "999" + ".pcd"
else:
    file_name = "../data/pcloud_filtered_passat/" + "999" + ".pcd"
o3d.io.write_point_cloud(file_name, point_cloud)
#o3d.visualization.draw_geometries([point_cloud, coord_frame], zoom=0.5, lookat=[0, 0, 1], front=[0, -0.2, -0.5], up=[0, -1, 0])
