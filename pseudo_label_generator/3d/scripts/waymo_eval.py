...
def _create_pd_file(frame, idx_abs, objects, labels_path):
    #txt_path = "/path/to/output/labels_waymo_corr_120/" + "segment-" + frame.context.name + "_with_camera_labels.tfrecord/" + str(idx) + ".txt"
    txt_path = labels_path + "segment-" + frame.context.name + "_with_camera_labels.tfrecord/" + str(idx_abs) + ".txt"
    detections = load_kitti_format(txt_path)

    for idx, laser_label in enumerate(detections):
        objects.objects.append(
            metrics_pb2.Object(
                object=label_pb2.Label(
                    box=label_pb2.Label.Box(
                        center_x=laser_label["location"][0],
                        center_y=laser_label["location"][1],
                        center_z=laser_label["location"][2],
                        length=laser_label["dimensions"][2],
                        width=laser_label["dimensions"][1],
                        height=laser_label["dimensions"][0],
                        heading=laser_label["rotation_y"]),
                    type=label_pb2.Label.TYPE_VEHICLE,
                    id=f'{idx_abs}_{idx}'),
                score=0.9,
...
def load_kitti_format(file_path):
    detections = []
    with open(file_path, 'r') as file:
        lines = file.readlines()
        for line in lines:
            split_line = line.strip().split(' ')
            detection_type = split_line[0]
            if detection_type not in ['Car', 'Cars']:
                continue
            detection = {
                'type': detection_type,
                ...
                'bbox': [float(val) for val in split_line[4:8]],
                'dimensions': [float(val) for val in split_line[8:11]],
                'location': [float(val) for val in split_line[11:14]],
                'rotation_y': float(split_line[14])
            }
            detections.append(detection)
    return detections

def create_bin(labels_path):
    dataset_path = "/path/to/open_waymo/OpenPCDet/data/waymo/raw_data/"
    dataset = tf.data.TFRecordDataset([str(x.absolute()) for x in Path(dataset_path).rglob('*.tfrecord')])
    objects = metrics_pb2.Objects()

    #with open("/path/to/waymo/ImageSets/train.txt", 'r') as file:
    with open("/path/to/open_waymo/OpenPCDet/data/waymo/ImageSets/train.txt", 'r') as file:
        lines = file.readlines()
    train = [line.strip() for line in lines]

    #check if inference labels already exist
    if os.path.exists(f'inference_labels.bin'):
        os.remove(f'inference_labels.bin')

    submission_file_base = 'inference_labels'  # Please modify accordingly.
    ...
    for data in tqdm.tqdm(dataset):
        ...
        if "segment-" + frame.context.name + "_with_camera_labels.tfrecord" not in train:
            continue
        if last_name != frame.context.name:
            last_name = frame.context.name
            idx = 0

        objects = _create_pd_file(frame, idx, objects, labels_path)

        idx += 1

    if os.path.exists(f'{submission_file_base}.bin'):
        os.remove(f'{submission_file_base}.bin')
    with tf.io.gfile.GFile(f'{submission_file_base}.bin', 'wb') as f:
...
def parse_metrics_objects_binary_files(ground_truths_path, predictions_path):
    ...
    for obj in ground_truth_objects.objects:
        ...
        ground_truth_box = obj.object.box
...
def main():
    ...
    create_bin(labels_path)

    WAYMO_OPEN_DATASET_DIR = '/path/to/waymo_scripts/2D_to_3D_Annotations/3d/scripts/'
    #WAYMO_OPEN_DATASET_DIR = '/path/to/2D_to_3D_Annotations/3d/scripts/'
    FAKE_GROUND_TRUTHS_BIN = (
            WAYMO_OPEN_DATASET_DIR + 'gt_labels_stride20.bin')
    FAKE_PREDICTIONS_BIN = (
            WAYMO_OPEN_DATASET_DIR + 'inference_labels.bin')
